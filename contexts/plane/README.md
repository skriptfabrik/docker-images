...
Customized [Plane](https://plane.so/) backend image based on the official
[`makeplane/plane-backend`](https://hub.docker.com/r/makeplane/plane-backend) image.

This context currently builds one image:

- [Dockerfile.backend](Dockerfile.backend) – the Plane backend (API server by default),
  published as `plane-backend`.

The image version follows the upstream `makeplane/plane-backend` version pinned in
[Dockerfile.backend](Dockerfile.backend) (`FROM makeplane/plane-backend:<version>`); this
is also what CI uses to derive the published image tags (see the
...
- **[`fileenv`](https://github.com/skriptfabrik/fileenv) entrypoint wrapper** – the
  ...
  as `ENTRYPOINT`, wrapping the original startup command (kept as `CMD`). Before starting
  the application, `fileenv` resolves any `<VAR>_FILE` environment variable into its
  corresponding `<VAR>` variable by reading the referenced file's contents (and unsets the
  `_FILE` variable afterwards) — this lets secrets (e.g. database credentials, the
  `SECRET_KEY`) be supplied via mounted files or Docker/Swarm secrets instead of plain
  environment variables, without requiring any change in Plane itself.

- **[docker-entrypoint.sh](docker-entrypoint.sh) connection URL builder** – runs before the
  upstream `docker-entrypoint-api.sh`, and assembles `AMQP_URL`, `DATABASE_URL`, and
  `REDIS_URL` from their individual component variables when the combined URL isn't
  already set:
...
The image runs as the Plane API server by default (`docker-entrypoint-api.sh`). Refer to
the [official Plane self-hosting documentation](https://developers.plane.so/self-hosting/overview)
for environment variables, the other backend roles (worker, beat-worker, migrator — run by
overriding `CMD` with the corresponding `bin/docker-entrypoint-*.sh` script), and general
configuration — this image does not change Plane's runtime behavior beyond the connection
URL defaults and `fileenv`-based secret resolution described above. Any environment
variable Plane supports can alternatively be supplied as `<VAR>_FILE` to have it resolved
